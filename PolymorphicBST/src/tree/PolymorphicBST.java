...
public class PolymorphicBST<K extends Comparable<K>, V>  {
	...
	public Set<K> keySet() {
		...
		HashSet<K> set = new HashSet<K>(root.size());
		root.addKeysToCollection(set);	//creates set of keys in tree
		return set;
	}
	 ...
	 */
	 public K getMin() {
		 ...
		 try {
			 ...
			 return root.min();
		 } catch(TreeIsEmptyException e) {	//if left subtree is empty
	 ...
	 * travesals of the tree.  Also you MAY not add an auxiliary method
